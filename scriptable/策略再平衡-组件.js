...
(() => {
  ...
  addText(heading, "策略再平衡", Font.boldSystemFont(13), new Color("#22312d"));
  heading.addSpacer();
  ...
  rows.forEach((strategy, index) => {
    if (index) widget.addSpacer(6);
    const row = widget.addStack();
    row.layoutHorizontally();
    const left = row.addStack();
    left.layoutVertically();
    addText(left, strategy.name, Font.mediumSystemFont(12), new Color("#283632"));
